...
Documento consolidado, já reconciliado com o seed real (Patch 2 aplicado):
usuário demo `@demo` no lugar de "Marina", `avg`/`count` explicitamente via
recompute, e os 4 enums confirmados como enums Postgres reais.

Ordem de execução = ordem das seções abaixo.
