...
/** Login mock -> onboarding -> /home, usando o usuário seed. */
async function loginAndOnboard(page: Page, { username = "mari.leituras" } = {}) {
  await page.goto("/");
  await page.click('a[href="/login"] >> nth=0');
  await page.waitForURL("**/login");
  await page.click('button:has-text("Continuar com Google")');
  await page.waitForURL("**/onboarding");
  await page.getByLabel("Nome", { exact: true }).fill("Marina Souza");
...
// C2: onboarding tem os placeholders esperados
test("onboarding mostra placeholders de nome, usuário e bio", async ({ page }) => {
  await page.goto("/");
  await page.click('a[href="/login"] >> nth=0');
  await page.waitForURL("**/login");
  await page.click('button:has-text("Continuar com Google")');
