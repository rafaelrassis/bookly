/** Dados de semeadura (dev/QA) — não são consumidos pelo app, só por
 * `prisma/seed.ts`. O catálogo real vem do Google Books em produção
 * (ver src/lib/books/google.ts); isto aqui só dá um ponto de partida
 * pro banco local ter conteúdo sem depender de cadastro manual. */
...
export const SEED_COMMUNITY_USERS: Record<string, SeedCommunityUser> = {
  "@ana.estante": {
    name: "Ana",
