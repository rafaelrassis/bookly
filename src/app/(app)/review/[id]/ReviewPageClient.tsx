...
export function ReviewPageClient({ params }: { params: { id: string } }) {
  ...
  if (status === "loading" || !review) return <ReviewLoading />;
    ...
    <div className="pt-4">
      ...
      <article className="mt-4">
        <div className="flex items-center gap-3">
          <Link href={profileHref} aria-label={authorHandle} className="tap rounded-full">
        ...
        <div className="mt-5 flex items-center gap-5 border-y border-line py-3 text-sm text-paperMuted">
          <button
            ...
            disabled={likeBusy}
            aria-pressed={liked}
            className={`transition-colors ${liked ? "text-ribbonText" : "hover:text-paper"}`}
          >
            ♥ {likeCount}
          </button>
