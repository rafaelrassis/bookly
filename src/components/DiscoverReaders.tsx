...
export function DiscoverReaders() {
  ...
  const suggestions = Object.entries(MOCK_USERS)
    .filter(([handle]) => handle !== withAt(myUsername) && !followed.includes(handle))
    .slice(0, 6);
    ...
    <section className="mt-7">
      ...
      <div className="no-scrollbar -mx-5 mt-3 flex gap-3 overflow-x-auto px-5">
        {suggestions.map(([handle, u]) => (
          ...
          >
            <Link href={`/u/${withoutAt(handle)}`} className="flex flex-col items-center gap-2">
            ...
            <button
              ...
              className="mt-3 w-full rounded-full bg-foil px-3 py-1.5 text-xs font-bold text-leather hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foil focus-visible:ring-offset-2 focus-visible:ring-offset-card"
            >
              Seguir
            </button>
          </div>
